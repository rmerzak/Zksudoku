...
npx snarkjs groth16 setup ${CIRCUIT}.r1cs ptau/powersOfTau28_hez_final_${PTAU}.ptau ${CIRCUIT}_0000.zkey

echo "Contibute to the 2 phase of the ceremony"
#Contribute to the phase 2 of the ceremony
npx snarkjs zkey contribute ${CIRCUIT}_0000.zkey ${CIRCUIT}_final.zkey --name="first contribution" -v -e="random"

echo "exporting the verification key"
#export the verification key
npx snarkjs zkey export verificationkey ${CIRCUIT}_final.zkey verificationkey.json

echo "Generate zk-proof"
#export a zk-proof associated to the circuit and the witness 
npx snarkjs groth16 prove ${CIRCUIT}_final.zkey ${CIRCUIT}_js/witness.wtns proof.json public.json

echo "Verify the proof"
#Verify the proof
npx snarkjs groth16 verify verificationkey.json public.json proof.json

echo "Generate Solidity verifier"
npx snarkjs zkey export solidityverifier ${CIRCUIT}_final.zkey ${CIRCUIT}Verifier.sol
